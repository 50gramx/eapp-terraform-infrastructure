...
output "ethos_control_plane_connection_string" {
  value = "'ssh -i ${var.ssh_private_key_file} ubuntu@${aws_eip.ethos_control_place_eip.public_dns}'"
}
...
output "ethos_worker_node_connection_string" {
  value = "'ssh -i ${var.ssh_private_key_file} ubuntu@${aws_eip.ethos_worker_node1_eip.public_dns}'"
}